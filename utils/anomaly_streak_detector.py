...
- 1.5x kritik eşik analizi.
- 2 Modlu yapıya uygun risk uyarıları.
"""
...
class AnomalyStreakDetector:
    ...
    def _analyze_extreme_streaks(self, history: List[float]) -> Dict[str, float]:
        ...
        if len(history) < 10:
            return {'has_extreme_above_streak': 0.0, 'has_extreme_below_streak': 0.0, 'extreme_streak_risk': 0.0}
        ...
        # Extreme streak risk
        if max_above >= 7 or max_below >= 7: extreme_risk = 0.9
        elif max_above >= 5 or max_below >= 5: extreme_risk = 0.7
        elif max_above >= 4 or max_below >= 4: extreme_risk = 0.5
        else: extreme_risk = 0.0
    ...
    def _calculate_break_probability(self, history: List[float]) -> float:
        ...
        current_streak_info = self._analyze_current_streak(history)
        current_streak = int(current_streak_info['current_streak_length'])
        
        # Streak ne kadar uzunsa, kırılma olasılığı o kadar yüksek (basit model)
        if current_streak >= 10: break_prob = 0.95
        elif current_streak >= 7: break_prob = 0.85
        elif current_streak >= 5: break_prob = 0.70
        elif current_streak >= 4: break_prob = 0.60
        elif current_streak >= 3: break_prob = 0.50
        else: break_prob = 0.30
        
        return float(break_prob)
    
    def _analyze_alternating_pattern(self, history: List[float]) -> Dict[str, float]:
        """Dalgalı hareket pattern'ini tespit eder (1.5 üstü/altı alternating)"""
        if len(history) < 5:
            return {'alternating_pattern_score': 0.0, 'is_alternating': 0.0}
    ...
    def _get_default_features(self) -> Dict[str, float]:
        """Default özellikler"""
        return {
            'current_above_streak': 0.0, 'current_below_streak': 0.0, 'current_streak_length': 0.0,
            'max_above_streak_10': 0.0, 'max_below_streak_10': 0.0,
            'max_above_streak_20': 0.0, 'max_below_streak_20': 0.0,
            'max_above_streak_50': 0.0, 'max_below_streak_50': 0.0,
            'has_extreme_above_streak': 0.0, 'has_extreme_below_streak': 0.0,
            'extreme_streak_risk': 0.0, 'streak_break_probability': 0.5,
            'alternating_pattern_score': 0.0, 'is_alternating': 0.0
        }
