"""
Feature Hash Validation System - Scaler Compatibility Checker (v2.0)

Bu modül, FeatureEngineering özellik sayısını doğrular ve 
scaler'ların model ile uyumlu olup olmadığını kontrol eder.

GÜNCELLEME:
- 2 Modlu (Normal/Rolling) sistem ile uyumlu hale getirildi.
- Minimum feature sayısı sınırı esnetildi (10+).
- Hash dosyası yedekleme mekanizması eklendi.
"""
...
class FeatureValidator:
    ...
    def _load_hashes(self):
        ...
        try:
            ...
            # Yedek varsa ondan yüklemeyi dene
            backup_path = f"{self.hash_file_path}.bak"
            if os.path.exists(backup_path):
                logger.info("🔄 Attempting to restore from backup...")
                try:
                    shutil.copy(backup_path, self.hash_file_path)
                    self._load_hashes() # Retry
                except:
                    self.feature_hashes = {}
                    self.scaler_hashes = {}
            else:
    ...
    def _save_hashes(self):
        ...
        try:
            ...
            if os.path.exists(self.hash_file_path):
                shutil.copy(self.hash_file_path, f"{self.hash_file_path}.bak")
    ...
    def generate_feature_hash(self, features: Dict[str, float]) -> str:
        ...
        try:
            ...
            # Sadece key isimlerini hashle (değerler değişebilir, yapı değişmemeli)
            feature_structure = json.dumps(sorted_keys)
            
            # SHA256 hash oluştur
            hash_object = hashlib.sha256(feature_structure.encode('utf-8'))
    ...
    def validate_feature_count(
        ...
        try:
            ...
            # GÜNCELLEME: Minimum sınır 10'a düşürüldü (hafif modeller için)
            if feature_count < 10: 
                return False, f"❌ Too few features: {feature_count} (minimum: 10)"
            
            if feature_count > 500: # Maximum sınır artırıldı
                return False, f"❌ Too many features: {feature_count} (maximum: 500)"
    ...
    def register_features(
        ...
        try:
            ...
            self.feature_hashes[model_name] = {
                ...
                'features_list': sorted(list(features.keys())) # Tüm listeyi sakla
            }
    ...
    def validate_compatibility(
        ...
        try:
            ...
            # Hash kontrolü (Kayıtlı model varsa)
...
def validate_model_compatibility(features: Dict[str, float], scaler, model_name: str = "default") -> Tuple[bool, str]:
    """Helper function"""
    validator = get_feature_validator()
    return validator.validate_compatibility(features, scaler, model_name)

def register_model_features(features: Dict[str, float], scaler, model_name: str = "default", version: str = "1.0"):
    """Helper function"""
    validator = get_feature_validator()
